...
type Billet = {
  id: string | number;
  // Propriétés attendues de l'API (type 'unknown' pour une vérification sûre)
  Titre?: unknown;
  Contenu?: unknown;
  Date?: unknown;
  // Champs de secours pour la compatibilité
  title?: string;
  body?: string;
  created_at?: string;
  date?: string;
    ...
    <main className="min-h-screen px-4 py-10">
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <header className="mb-8">
          {/* Titre principal */}
          <h1 className="text-3xl font-semibold text-cyan-500">Liste des billets</h1>
        ...
        {errorMessage ? (
          ...
          <ul className="space-y-4">
            ...
            {billets.map((billet, index) => {
              ...
              const title = typeof billet.Titre === 'string' && billet.Titre ? billet.Titre : billet.title ?? `Billet non titré ${index + 1}`;
              const body = typeof billet.Contenu === 'string' ? billet.Contenu : billet.body;
              // Identifiant : utilise billet.id ou l'index comme fallback
              const id = String(billet.id ?? index + 1);
              // Date de création : utilise billet.Date ou une valeur par défaut
              const date = typeof billet.Date === 'string' ? billet.Date :
                           typeof billet.created_at === 'string' ? billet.created_at :
                           typeof billet.date === 'string' ? billet.date :
                           'Date inconnue';
              ...
              return (
                ...
                >
                  ...
                  {/* Date de création */}
                  <p className="mt-1 text-sm text-slate-500">Crée le : {date}</p>
